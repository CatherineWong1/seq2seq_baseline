...
def _get_random_bucket(train_buckets_scale):
    """
    Get a random bucket from which to choose a traning sample
    :param train_buckets_scale:
    :return:
    """
    rand = random.random()
...
def run_step(sess, model, encoder_inputs, decoder_inputs, decoder_masks, bucket_id, forward_only):
    ...
    forward_only is set to True when you just want to evaluate on the test set,or
    when you want to the bot to be in chat mode
    """
...
def _construct_response(output_logits, inv_dec_vocab):
    ...
    res = " ".join([tf.compat.as_str(inv_dec_vocab[output]) for output in outputs])
    print("****************209*************")
    print(res)
    return res
